...
getopts ('n:htvDc:',\%ARGS);
if ( $ARGS{'D'} && ! $ARGS{'v'} ) {
	$ARGS{'v'}++;
}
if ( $ARGS{'v'} ) {
	print "rsync-backup.pl Copyright (c) 2003-2012 Greg Boyington.\n";
	print "Beginning test run as UID $>...\n" if $ARGS{'t'};
}
if ( $ARGS{'h'} ) {
	die "Usage: $0 [-v | -D ] [ -t ] [ -h ] [ -n num ] [ -c config_dir ] [ label... ]\nperldoc $0 for detailed help.\n";
}
...
my $config_dir = $ARGS{'c'}||'rsync-targets';
print "Parsing configuration directory '$config_dir'...\n" if $ARGS{'v'};
my $conf = &readConfig($config_dir);
...
die "$config_dir contains no valid config blocks; exiting.\n"
	unless keys %$conf;
...
sub launch_backup() {
	...
	notify \*LOG, "[$label] Checking directory hierarchy...\n";
	foreach my $d ( $c->{'snapshot-path'},
		...
		if ( ! -d $d ) {
			mkdir $d or fail(\*LOG, "[$label] Couldn't create $d: $!");
		}
	}
...
sub readConfig {
	...
	fail "Couldn't read config dir $dir: $!"
		unless -d $dir;

	my @files;
	opendir(DIR, $dir)
		or fail "Couldn't read config dir $dir: $!";
	push @files, $_ foreach grep { !/^\./ } readdir(DIR);
	closedir(DIR);
	...
	foreach my $file ( @files ) {
		fail "Couldn't read config file $dir/$file: $!" 
			unless -f "$dir/$file";

		open (CONF,"$dir/$file")
			or fail "Couldn't open config file $file for reading: $!";
...
rsync-backup.pl [ SWITCHES ] [ -f /path/to/config_dir ] [ label label... ]
...
=item B<-c>      Path to the configuration directory
...
You may have as many such config blocks in your config files, and as 
many files in your configuration directory, as you like; 
...
by the highest value of snapshots-daily in your config files.  For
example, the config block shown above would suggest the following crontab
entry:

=over

=item 0 0,6,12,18 * * *  /path/to/rsync-backups.pl -f conf_dir
...
=item -- replaced -f arg with -c for configuration directories
